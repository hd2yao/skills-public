...
description: 适用于需要在用户本机的真实 Google Chrome 中打开腾讯广告报表链接、把登录交接给用户，并通过本机脚本继续执行下载流程，不依赖特定 AI Agent 平台提供的浏览器插件、内置浏览器或桌面接管能力。
---

# 腾讯广告报表下载

用用户本机的真实 `Google Chrome` 执行腾讯广告报表下载流程，不依赖特定 AI Agent 平台提供的浏览器插件、内置浏览器或桌面接管能力。
...
- 不使用特定 AI Agent 平台提供的浏览器插件、浏览器扩展、claim tab 或其他依赖平台接管标签页的自动化方式
- 不使用 `Computer Use`，也不使用特定 AI Agent 平台提供的桌面点击、读屏或远程接管能力
- 不把最终下载留在任何平台的内置浏览器中执行
- 不尝试代替用户完成微信扫码、QQ 授权、短信验证码或二次确认

如果系统级 Chrome 不可用，明确告知用户并停止，不要偷偷切回任何平台的内置浏览器。
...
不要使用任何特定 AI Agent 平台的浏览器或桌面接管能力代替上面两层。
