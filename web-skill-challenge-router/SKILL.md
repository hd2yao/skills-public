...
| arithmetic expression image or text | `arithmetic` | manual until skill exists |
| image text captcha | `image_captcha` | manual until skill exists |
| SMS or phone code | `sms_otp` | manual takeover |
| email code | `email_otp` | manual takeover |
| QR scan | `qr_scan` | manual takeover |
| SSO consent, device approval, app confirmation | `external_approval` | manual takeover |
| cannot classify | `unknown` | manual takeover |

第一版只有 `slider_puzzle` 可以自动路由。后续新增验证 Skill 时，只扩展这张表和输出路由，不修改主编排流程。
