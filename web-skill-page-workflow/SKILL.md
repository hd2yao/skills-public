...
## Visible Table Selection

当用户说“最上面”“第一条”“当前列表”“当前表格”“任务状态是 X 的制作编号”时，必须以当前可见表格为准，而不是以 DOM 全量文本、隐藏面板、旧详情区或滚动区域外内容为准。

执行顺序：

1. 先确认当前选中的 tab、筛选状态和表格标题，例如 `待跟进制作`。
2. 截图或读取可见 DOM，确认用户看到的表格行。
3. 只在当前可见表格的 body 行中匹配条件。
4. 如果条件是“任务状态 是 已开始”，先找到状态列，再取同一行的制作编号列。
5. 如果页面下方已有旧详情区，不要把旧详情区里的制作编号当成当前列表选择结果。
6. 点击前记录选择依据，例如：`选择当前可见表格第一行，制作编号 523101，任务状态 已开始`。
7. 点击后必须验证详情标题或详情区制作编号与所选制作编号一致。

如果自动化快照和用户截图冲突，以用户当前截图/可见页面为准，重新采集当前可见状态；不要继续使用旧快照里的行号或制作编号。
...
### Picking from stale DOM instead of the visible table

有些后台会同时保留列表、旧详情区、抽屉、隐藏面板或缓存 DOM。用户说“最上面”时，必须先限定到当前可见表格，再按行和列取值；不能从页面全文中找到一个匹配编号就点击。
